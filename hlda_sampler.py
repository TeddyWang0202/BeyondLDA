...
class NCRPNode(object):
    ...
    def get_top_words(self, n_words, with_weight):
        ...
        return output
    ### Modify ###
    def get_topic_word(self, with_weight):
        pos = np.argsort(self.word_counts)[::-1]
        sorted_weights = self.word_counts[pos]
        sorted_weights = sorted_weights[:]
        sorted_weights = np.trim_zeros(sorted_weights, 'b')
        sorted_vocab = self.vocab[pos]
        sorted_vocab = sorted_vocab[:len(sorted_weights)]
        topic_word = {k:v for k, v in zip(sorted_vocab, sorted_weights)}
        return topic_word
